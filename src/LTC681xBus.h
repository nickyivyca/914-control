...
class LTC681xBus {
 public:
  ...
  static Command buildAddressedCommand(uint8_t addr, LTC681xCommand const &cmd);
  static Command buildBroadcastCommand(LTC681xCommand const &cmd);

  LTC681xBus(SPI *spiDriver);
  ...
  void sendCommandWholeChain(Command txCmd, uint8_t txData[NUM_CHIPS][6]);
  uint8_t readWholeChainCommand(Command txCmd, uint8_t rxbuf[NUM_CHIPS][8]);
